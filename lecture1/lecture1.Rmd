...
is.numeric(x)
is.factor(z)
is.numeric(z) # since we asked about the class, not the mode
```
...
Be very careful with matrices!
```{r, echo=F}
cbind(c(1,2), c("a", "b"))
data.frame(c(1,2), c("a", "b"))
data.frame("ID"=c(1,2), "name"=c("a", "b")) # better
```
...
Lists store information of many different types

```{r}
amy <- list(office = "f657", hikes = TRUE, scores = c(7, 9))
amy
```
...
```{r}
plot
methods("plot") # lists all types that R knows how to plot
```
